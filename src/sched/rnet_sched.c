/* rnet_sched.c — rollback admission scheduler (policy only).
 *
 * Lifted from MotK psx_netplay_sched.c into retcomm-rbengine, then moved here:
 * every decision it makes is about peers, so it belongs with the session and
 * the episode FSM rather than in the offline rollback core. Host pacing /
 ...
 * The RBE_ names predate the move into recomp-net and are kept so existing
 * tuning and soak scripts keep working. */
static const char *rnet_env(const char *primary, const char *legacy)
{
...
static int rnet_sched_rollback_active(void)
{
...
int rnet_sched_real_delay_enabled(void)
{
    static int s_mode = -1;
    if (s_mode < 0) {
        const char *e = rnet_env("RBE_RB_ZERO_DELAY", "PSX_RB_ZERO_DELAY");
...
{
    int d;
    if (rnet_sched_real_delay_enabled())
        return sim_tick;
    d = sched_delay();
    if (d < 0)
        d = 0;
...
void rnet_sched_sync_delay_from_session(void)
{
    ...
    if (d >= 2u && d < (rnet_u8)RB_AUTO_DELAY_LOWER_FLOOR &&
        rnet_sched_rollback_active() && rnet_sched_real_delay_enabled()) {
 ...
 * the admit up to the grace budget measured from the first miss of that
 * wire tick. RBE_RB_INVENT_GRACE_MS sets the floor (default 8 ms); the
 * per-stall budget scales up from RTT in np_invent_grace_stall_ex. This
 * is a rate governor, not just packet wait: while the ahead peer stalls
 * here, the behind peer keeps simulating and catches up, so in steady
 * state the sims stay aligned and inputs
                                    ...
                                    int count_expire)
{
    ...
    static rnet_u32 s_wire[RNET_SCHED_MAX_SLOTS];
    static uint32_t s_t0[RNET_SCHED_MAX_SLOTS];
    static uint8_t  s_expired[RNET_SCHED_MAX_SLOTS];
    static uint8_t  s_budget_logged[RNET_SCHED_MAX_SLOTS];
    ...
    if (s_grace_ms < 0) {
        const char *e = rnet_env("RBE_RB_INVENT_GRACE_MS", "PSX_RB_INVENT_GRACE_MS");
    ...
    if (s_grace_ms == 0 || budget_cap == 0u || slot < 0 || slot >= RNET_SCHED_MAX_SLOTS)
        return 0;
    now = sched_mono_ms();
...
static uint32_t np_gap1_grace_cap_ms(int *has_override)
{
    ...
    if (s_cap == -2) {
        const char *e = rnet_env("RBE_RB_GAP1_GRACE_MS", "PSX_RB_GAP1_GRACE_MS");
        if (e && e[0]) {
            s_cap = atoi(e);
            if (s_cap < 0) s_cap = 0;
...
{
    if (g_ts_enabled < 0) {
        const char *e = rnet_env("RBE_RB_TIMESYNC", "PSX_RB_TIMESYNC");
...
static int np_cross_os_diag_enabled(void)
{
    static int s = -1;
    if (s < 0) {
        const char *e = rnet_env("RBE_CROSS_OS_PACING_DIAG", "PSX_NETPLAY_CROSS_OS_PACING_DIAG");
        s = (e && e[0] == '1' && e[1] == '\0') ? 1 : 0;
    }
    return s;
}
                                  ...
                                  const RNetSessionStats *st)
{
    ...
    if (s_last_ms != 0u && (uint32_t)(now - s_last_ms) < 1000u)
        return;
    s_last_ms = now ? now : 1u;
    stall = rnet_sched_admit_stall_tag();
...
static void np_adapt_delay_on_pcap_enter(uint32_t now)
{
    ...
    if (g_adapt_delay_enabled < 0) {
        e = rnet_env("RBE_RB_ADAPT_DELAY", "PSX_RB_ADAPT_DELAY");
...
static void np_auto_delay_tick(uint32_t now)
{
    ...
    if (s_enabled < 0) {
        const char *e = rnet_env("RBE_RB_AUTO_DELAY", "PSX_RB_AUTO_DELAY");
                              ...
                              const RNetSessionStats *st, const char *stall_why)
{
    ...
    rnet_sched_set_admit_stall("wire_hole");
}
...
int rnet_sched_pre_admit(uint32_t sim, uint32_t wire, const RNetSessionStats *st)
{
...

    {
        ...
        if (sched_pre_admit_hold(sim, wire, &hold_tag)) {
            rnet_sched_set_admit_stall(hold_tag && hold_tag[0] ? hold_tag
                                                              : "pre_admit_hold");
    ...
    if (!sched_episode_active() && np_timesync_throttle(wire)) {
        rnet_sched_set_admit_stall("timesync_pace");
        np_cross_os_maybe_log(sched_mono_ms(), sim, st);
        return 1;
    }
                            ...
                            const char **reason_out)
{
    ...
    if (sched_lockstep_no_invent()) {
        ...
        if (st->highest_remote_wire > wire)
            ...
            rnet_sched_set_admit_stall(why);
        if (reason_out)
            *reason_out = why;
        return 1;
    }
     ...
     * production tip (sim0 tip == D) before inventing. */
    {
        ...
        if (st->highest_remote_wire <= (rnet_u32)d) {
            ...
            rnet_sched_set_admit_stall("boot_tip_wait");
            if (reason_out)
    ...
    if (wire > st->highest_remote_wire + (rnet_u32)pred) {
        ...
        rnet_sched_set_admit_stall("pcap_freeze");
        if (reason_out)
    ...
    if (g_cushion_rebuild && !sched_episode_active()) {
        ...
        if (absurd_invent_catchup_active(now_c) && st->remote_lead > max_ok) {
            ...
            rnet_sched_set_admit_stall("absurd_catchup");
        } else {
            if (st->highest_remote_wire > wire)
                np_diag_wire_hole(slot, sim, wire, st, "cushion_rebuild");
            else
                rnet_sched_set_admit_stall("cushion_rebuild");
            if (reason_out)
    ...
    if (s_gap1_legacy < 0) {
        const char *e = rnet_env("RBE_RB_GAP1_INVENT", "PSX_RB_GAP1_INVENT");
    ...
    if (!s_gap1_legacy && st->remote_lead > 0) {
        ...
        if ((uint32_t)(now_miss - s_miss_t0) < tip_stale_ms) {
            ...
            if (st->highest_remote_wire > wire)
                ...
                rnet_sched_set_admit_stall("tip_stale_wait");
            if (reason_out)
        ...
        if (gap1_cap != 0u) {
            if (np_invent_grace_stall_ex(slot, wire, gap1_cap, 0)) {
                ...
                rnet_sched_set_admit_stall("gap1_grace");
                if (reason_out)
        ...
        if (pred_depth >= 2u &&
            ...
            rnet_sched_set_admit_stall("depth_stale_wait");
            if (reason_out)
        ...
        if (np_invent_grace_stall_ex(slot, wire,
            ...
            if (st->highest_remote_wire > wire)
                ...
                rnet_sched_set_admit_stall("runway_grace");
            if (reason_out)
    ...
    rnet_sched_clear_admit_stall(); /* inventing — not a barrier stall */
    return 0;
}
